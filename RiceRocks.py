...
class Ship:
    ...
    def set_thrust (self, on):
        global started
        self.thrust = on
        if on and started:ship_thrust_sound.rewind();ship_thrust_sound.play ()
...
class Sprite:
    ...
    def collide (self, other):
        if dist (self.pos, other.get_position ()) <= self.radius + other.get_radius ():return True
        else:return False
        
def keydown (key):
    if key == simplegui.KEY_MAP['left']:    my_ship.decrement_angle_vel ()
    elif key == simplegui.KEY_MAP['right']: my_ship.increment_angle_vel ()
    elif key == simplegui.KEY_MAP['up']:    my_ship.set_thrust (True)
    elif key == simplegui.KEY_MAP['space']: my_ship.shoot ()
        
def keyup (key):
    if key == simplegui.KEY_MAP['left']:    my_ship.increment_angle_vel ()
    elif key == simplegui.KEY_MAP['right']: my_ship.decrement_angle_vel ()
    elif key == simplegui.KEY_MAP['up']:    my_ship.set_thrust (False)
...
def click (pos):
    ...
    if  inwidth ==True and inheight==True and (not started):
        started = True
        score ,lives,MAX_ROCK= 0,3,12
        ship_thrust_sound.rewind ();explosion_sound.rewind ();missile_sound.rewind ();soundtrack.rewind ();soundtrack.play ();

def screen_images(canvas):
...
def process_sprite_group (group, canvas):
    removeSet = set ([])
    for sprite in group:
        if sprite.update ():removeSet.add (sprite)
        else:sprite.draw (canvas)
    if len (removeSet) > 0:group.difference_update (removeSet)
